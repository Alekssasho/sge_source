...
struct Player : public Actor {
	...
	void create() {
		...
		ttModel.setModel("assets/models/playerBall.mdl", true);
		ttRigidbody.getRigidBody()->create(this, CollsionShapeDesc::createSphere(1.f), 1.f, false);
		ttRigidbody.getRigidBody()->setRollingFriction(0.075f);
		ttRigidbody.getRigidBody()->setSpinningFriction(0.075f);
	...
	void update(const GameUpdateSets& u) {
		...
		ttRigidbody.getRigidBody()->applyForce(inputDirWs * 500.f * u.dt, vec3f(0.f, 0.65f, 0.f));
	}
};
