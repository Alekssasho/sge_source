...
void HLSLGenerator::OutputExpression(HLSLExpression* expression)
{
    if (expression->nodeType == HLSLNodeType_IdentifierExpression)
    {
        ...
        if (!m_legacy && IsSamplerType(identifierExpression->expressionType) && identifierExpression->global)
        {
            ...
            {
                m_writer.Write("%s(%s, %s_sampler)", m_textureSampler2DCtor, name, name);
            }
            else if (identifierExpression->expressionType.baseType == HLSLBaseType_Sampler3D)
            {
                m_writer.Write("%s(%s, %s_sampler)", m_textureSampler3DCtor, name, name);
            }
            else if (identifierExpression->expressionType.baseType == HLSLBaseType_SamplerCube)
            {
                m_writer.Write("%s(%s, %s_sampler)", m_textureSamplerCubeCtor, name, name);
            }
            else if (identifierExpression->expressionType.baseType == HLSLBaseType_Sampler2DShadow)
            {
                m_writer.Write("%s(%s, %s_sampler)", m_textureSampler2DShadowCtor, name, name);
            }
    ...
    else if (expression->nodeType == HLSLNodeType_ArrayAccess)
    {
		...
		if(arrayAccess->expressionType.baseType == HLSLBaseType_Sampler2D) {
			//OutputExpression(arrayAccess->array);
			// HACK:
			if (arrayAccess->array->nodeType == HLSLNodeType_IdentifierExpression)
			{
				HLSLIdentifierExpression* identifierExpression = static_cast<HLSLIdentifierExpression*>(arrayAccess->array);
				const char* name = identifierExpression->name;
				 if (identifierExpression->expressionType.baseType == HLSLBaseType_Sampler2D)
				{
					 m_writer.Write("%s(%s[", m_textureSampler2DCtor, name);
					 OutputExpression(arrayAccess->index);
					 m_writer.Write("], %s_sampler[", name);
					 OutputExpression(arrayAccess->index);
					 m_writer.Write("])");
				}
			}
        ...
        if (samplerType != NULL)
        {
            if (reg != -1)
            {
				if(isArray) {
					...
					OutputExpression(declaration->type.arraySize);
					m_writer.Write("] : register(t%d);", reg);

					m_writer.Write("%s %s_sampler[", samplerType, declaration->name);
					OutputExpression(declaration->type.arraySize);
					m_writer.Write("] : register(s%d);", reg);
				}
				else
				{
					m_writer.Write("%s %s : register(t%d); %s %s_sampler : register(s%d)", textureType, declaration->name, reg, samplerType, declaration->name, reg);
				}
            ...
            else
            {
				...
				{
					m_writer.Write("%s %s; %s %s_sampler", textureType, declaration->name, samplerType, declaration->name);
				}
            }
        }
        else
