...
#include "matrix_common.inl"
	...
	static SELF_TYPE getRotationQuat(const quat<DATA_TYPE>& q) {
		...
		result.data[0] = VEC_TYPE(one - two * (y * y + z * z), two * (x * y + z * w), two * (x * z - y * w));

		result.data[1] = VEC_TYPE(two * (x * y - z * w), one - two * (x * x + z * z), two * (y * z + x * w));

		result.data[2] = VEC_TYPE(two * (x * z + y * w), two * (y * z - x * w), one - two * (x * x + y * y));
