...
struct SGE_CORE_API DebugDraw {
	...
	struct Group {
		...
		void clear(bool deallocMem) { m_wieredData.clear(deallocMem); }
  ...
  private:
	void drawWieredCommand(const RenderDestination& rdest, const mat4f& projView, const WiredCommandData& cmd);
