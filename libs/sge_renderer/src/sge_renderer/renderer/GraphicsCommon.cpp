...
size_t TextureFormat::GetSizeBits(const TextureFormat::Enum format)
{
	switch(format)
	{
		...
		case BC1_UNORM: return 8;
		case BC2_UNORM: return 8;
		case BC3_UNORM: return 8;

		case BC4_UNORM: return 8;
		case BC4_SNORM: return 8;

		case BC5_UNORM: return 16;
		case BC5_SNORM: return 16;
	};
...
int UniformType::GetSizeBytes(const UniformType::Enum uniformType)
{
	switch(uniformType)
	{
		...
		case Int_RGBA_Unorm_IA: return 4;
	};
...
// Buffers
//-------------------------------------------------------------------------------
BufferDesc BufferDesc::GetDefaultVertexBuffer(
	const size_t sizeBytes,
	const ResourceUsage::Enum usage)
{
	BufferDesc result;
...
BufferDesc BufferDesc::GetDefaultIndexBuffer(
	const size_t sizeBytes,
	const ResourceUsage::Enum usage)
{
	BufferDesc result;
...
BufferDesc BufferDesc::GetDefaultConstantBuffer(
	const size_t sizeBytes,
	const ResourceUsage::Enum usage)
{
	BufferDesc result;
...
// PrimitiveTopology
//-------------------------------------------------------------------
int PrimitiveTopology::GetNumPrimitivesByPoints(
	const PrimitiveTopology::Enum topology,
	const int numPoints)
{
	switch(topology)
	{
	case Unknown: sgeAssert(false); return 0;
	case TriangleList: return numPoints / 3;
	case TriangleStrip: return numPoints > 2 ? numPoints - 2 : 0;
	case LineList: return numPoints / 2;
	case LineStrip: return numPoints - 1;
	case PointList: return numPoints;
	}
...
int PrimitiveTopology::GetNumPointsByPrimitives(
	const PrimitiveTopology::Enum topology,
	const int numPrimitives)
{
	if(numPrimitives == 0) return 0;
...
std::vector<VertexDecl> VertexDecl::NormalizeDecl(const VertexDecl* pDecl, const int numDeclElems)
{
	...
	if(numDeclElems <= 0) return std::vector<VertexDecl>();
	...
	std::sort(result.begin(), result.end(),
		[](const VertexDecl & a, const VertexDecl & b) -> bool
	{
...
bool BlendDesc::operator==(const BlendDesc& other) const
{
	return 
		...
		alphaDestBlend == other.alphaDestBlend &&
