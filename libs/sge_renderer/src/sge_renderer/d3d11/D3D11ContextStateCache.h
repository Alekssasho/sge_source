...
public : 
	...
	void TextureUnbind(
		ID3D11ShaderResourceView * const srvs[], const int srvCnt,
		ID3D11RenderTargetView* const rtvs[], const int rtvCnt,
		ID3D11DepthStencilView* const dsvs[], const int dsvCnt
	);
...
public : 
	...
	struct BoundVertexBuffers
	{
		BoundVertexBuffers()
		{
			for(auto& v: buffer) v = NULL;
			for(auto& v: stride) v = 0;
			for(auto& v: byteOffset) v = 0;
		}
	...
	struct ShadingStageResources
	{
		...
		{
			for(auto& v : cbuffers) v = nullptr;
			for(auto& v : srvs) v = nullptr;
			for(auto& v : samplerStates) v = nullptr;
		}
